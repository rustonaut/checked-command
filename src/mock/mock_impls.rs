...
use super::{ChildMock, ProcessInputMock, ProcessOutputMock, Spawner};
...
where
    F: 'static
        + Send
        + Sync
        + Fn(SpawnOptions, bool, bool) -> Result<Box<dyn ChildMock>, io::Error>,
{
    func: F,
}
...
impl MockResult {
    /// Creates a new instance returning it as a boxed trait object.
    pub fn new(result: Result<ExecResult, io::Error>) -> Box<dyn ChildMock> {
        Box::new(MockResult { result })
    }
}

impl ChildMock for MockResult {
...
{
    /// Creates a new instance returning it as a boxed trait object.
    pub fn new(func: F) -> Box<dyn ChildMock> {
        Box::new(MockResultFn { func })
    }
}

impl<F> ChildMock for MockResultFn<F>
where
    F: 'static + Send + FnOnce() -> Result<ExecResult, io::Error>,
{
    fn take_stdout(&mut self) -> Option<Box<dyn ProcessOutputMock>> {
